...
def test_get_search_results_sans_annee(service_donnees):
    ...
    # Act
    resultats = service_donnees.get_search_results(year=None, dept='INFO', rythme='FI', regles_sql="")
...
def test_get_search_results_annee_invalide(service_donnees):
    ...
    # Act
    resultats = service_donnees.get_search_results(year="vingt", dept='INFO', rythme='FI', regles_sql="")
...
def test_get_search_results_succes(service_donnees):
    ...
    # Act
    resultats = service_donnees.get_search_results(year="2023", dept='INFO', rythme='FI', regles_sql="")

    # Assert
    assert len(resultats) == 1
    # On vérifie que la ligne SQL a bien été transformée en objet (on peut appeler .ine)
    assert resultats[0].ine == '123456789AZ'
